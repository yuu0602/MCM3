...
from config import PROMOTER_OVERLAP_BP, RATIO_MIN
...
def promoter_gene_groups(sets: dict[str, set[str]]) -> dict[int, set[str]]:
    """Partition MCM3-bound genes by passing promoter criteria, not peak tails."""
    return {
        mask: {
            gene for gene in sets["MCM3"]
            if sum(1 << index for index, factor in enumerate(FACTORS) if gene in sets[factor]) == mask
        }
        for mask in (1, 3, 5, 7)
    }
...
def peak_promoter_associations(groups: dict[int, pd.DataFrame]) -> pd.DataFrame:
    """Annotate peak loci using the configured promoter-overlap rule."""
...
def write_peak_profile_summary(rows: list[dict[str, object]]) -> None:
    """Replace metadata for the two gene-level profiles, retaining other panels."""
    ...
    summary = replacement if previous.empty else pd.concat((previous, replacement), ignore_index=True, sort=False)
...
def write_profile_gene_bed(genes: set[str], path: Path) -> int:
    """Write one canonical gene-body record per promoter-bound gene."""
    bodies = pd.read_csv(
    ...
    if missing:
        raise RuntimeError(f"{len(missing)} promoter-bound genes are missing from the M25 gene-body table")
...
def render_promoter_bound_gene_profiles() -> None:
    """Render disjoint promoter-bound gene groups with one observation per gene."""
    OUT.mkdir(parents=True, exist_ok=True)
    sets = promoter_sets()
    groups = promoter_gene_groups(sets)
    if sum(map(len, groups.values())) != len(sets["MCM3"]):
        raise RuntimeError("Promoter groups must partition the MCM3-bound gene set")
    associations = peak_promoter_associations(peak_locus_groups())
    associations["promoter_binding_mask"] = associations.gene.map(
        lambda gene: sum(1 << i for i, factor in enumerate(FACTORS) if gene in sets[factor])
    )
    associations["role"] = "peak annotation only; not gene-profile membership"
    ...
    if any(not genes for genes in groups.values()):
        empty = [str(mask) for mask, genes in groups.items() if not genes]
        raise RuntimeError("No promoter-bound genes for group(s): " + ", ".join(empty))
    ...
    group_specs = (
        ("All three factors", 7, COLORS["MCM3"]),
        ("MCM3-NONO only", 3, COLORS["NONO"]),
        ("MCM3-PSPC1 only", 5, COLORS["PSPC1"]),
        ("MCM3 only", 1, "#777777"),
    )
    gene_table = pd.DataFrame([
        {"gene": gene, "promoter_binding_mask": mask, "group": label,
         **{factor: gene in sets[factor] for factor in FACTORS}}
        for label, mask, _ in group_specs for gene in sorted(groups[mask])
    ])
    gene_table.to_csv(CUTRUN / "data" / "MCM3_Peak_profiles_genes.tsv", sep="\t", index=False)
    gene_table.loc[gene_table.promoter_binding_mask.eq(7)].to_csv(
        CUTRUN / "data" / "MCM3_NONO_PSPC1_peak_profiles_genes.tsv", sep="\t", index=False
    )
    ...
    for label, mask, color in group_specs:
        genes = groups[mask]
        gene_bed = OUT / f"MCM3_Peak_profiles_region_{mask:03b}_regions.bed"
        write_profile_gene_bed(genes, gene_bed)
        names, values = matrix(OUT / f"MCM3_Peak_profiles_region_{mask:03b}_matrix.gz", gene_bed, [TRACKS["MCM3"]], "scale-regions")
        profile = values[:, 0, :].mean(axis=0)
        retained = values.shape[0]
        if retained != len(genes) or set(names) != genes:
            raise RuntimeError(f"{label}: matrix rows do not match the unique promoter-bound genes")
        ...
        summary_rows.append({
            ...
            "unit": "unique promoter-bound genes", "n_input_genes": len(genes),
            "n_profile_genes": retained, "promoter_overlap_rule": f">={PROMOTER_OVERLAP_BP} bp",
            "membership_source": "05_promoters/Venn_PromoterGenes_{factor}.tsv",
            "replicate_support": f"factor/IgG >={RATIO_MIN:g} in each of 2 replicates",
    ...
    triple_genes = groups[7]
    triple_bed = OUT / "MCM3_NONO_PSPC1_peak_profiles_regions.bed"
    write_profile_gene_bed(triple_genes, triple_bed)
    names, matrix_values = matrix(OUT / "MCM3_NONO_PSPC1_peak_profiles_matrix.gz", triple_bed, [TRACKS[factor] for factor in FACTORS], "scale-regions")
    values = {factor: matrix_values[:, index, :].mean(axis=0) for index, factor in enumerate(FACTORS)}
    retained = matrix_values.shape[0]
    if retained != len(triple_genes) or set(names) != triple_genes:
        raise RuntimeError("Triple promoter-bound matrix rows do not match the gene set")
    fig = plt.figure(figsize=(7.4, 5.6), dpi=300, facecolor="white")
    ...
    summary_rows.append({
        ...
        "unit": "unique promoter-bound genes", "n_input_genes": len(triple_genes),
        "n_profile_genes": retained, "promoter_overlap_rule": f">={PROMOTER_OVERLAP_BP} bp",
        "membership_source": "05_promoters/Venn_PromoterGenes_{factor}.tsv",
        "replicate_support": f"factor/IgG >={RATIO_MIN:g} in each of 2 replicates",
        "reference_point": "TSS/TES scaled gene body",
...
def render_promoter_gene_profile() -> None:
    ...
    axis.text(
        .5, 1.012,
        f"p ≤ 1e-4; q ≤ 0.01; ratio ≥{RATIO_MIN:g} in 2/2; overlap ≥{PROMOTER_OVERLAP_BP} bp; n={n_genes:,}",
...
def main() -> None:
    ...
    render_promoter_bound_gene_profiles()
