...
def main() -> None:
    ...
    parser.add_argument("--promoter-groups-only", action="store_true", help="Update only the two gene-group profiles and their source tables")
    args = parser.parse_args()
    if args.dry_run:
        print("[DRY-RUN] Would render the registered CUT&RUN figures.")
        return
    if args.promoter_groups_only:
        if args.publish_only:
            parser.error("--promoter-groups-only cannot be combined with --publish-only")
        configure_additional().render_promoter_bound_gene_profiles()
        print(f"[DONE] Promoter-bound gene-group profiles: {VISUALS}")
        return
