...
def atomic_peak_segments(
    ...
    for chrom, chrom_events in events.items():
        ...
        for position, kind, bit in sorted(
            ...
            if (
                previous is not None
                and position - previous >= ATOMIC_SEGMENT_MIN_BP
                and active
            ):
...
def install_segments(
    ...
    counts["minimum_peak_overlap_bp"] = ATOMIC_SEGMENT_MIN_BP
        ...
        summary.append(
            {
                ...
                "rule": f"pooled matched-IgG peak; promoter ratio >={RATIO_MIN:g} in 2/2 replicates",
            }
        )
    ...
    write_peak_parameters()


def write_peak_parameters() -> None:
    pd.DataFrame([
        ("unit", "exact atomic genomic segment"),
        ("minimum_shared_segment_bp", ATOMIC_SEGMENT_MIN_BP),
        ("membership_rule", "true simultaneous interval support; no connected-component chaining"),
        ("source", "MAPQ>=30 matched-IgG MACS3 pooled peaks; retained p<=1e-4; q<=0.01; FE>=3"),
        ("pie_comparison", "peak-distribution pies use the same factor-specific atomic segments"),
        ("regulatory_target_role", "descriptive only; not used for promoter-bound or DEG intersections"),
    ], columns=["parameter", "value"]).to_csv(
        CUTRUN_ROOT / "data/figure_inputs/Venn_Peaks_parameters.tsv", sep="\t", index=False
    )
...
def install_promoters(
        ...
        [
            {
                "factor": factor,
                "promoter_bound_genes": len(promoter_sets[factor]),
                "rule": f"pooled matched-IgG peak plus factor/IgG >={RATIO_MIN:g} in 2/2 replicates",
            }
            for factor in FACTORS
        ]
    )
    ...
    parameters = pd.DataFrame(
        [
            ...
            ("peak_promoter_overlap", f">={PROMOTER_OVERLAP_BP} bp"),
            ("atomic_segment_minimum", f">={ATOMIC_SEGMENT_MIN_BP} bp"),
            (
                "binding_rule",
                f"passing pooled peak plus promoter factor/IgG mean >={RATIO_MIN:g} in 2/2 biological replicates",
            ),
...
def raw(manifest: pd.DataFrame, reuse_peaks: bool = False) -> None:
    macs3 = executable(
    ...
    for factor in FACTORS:
        ...
        candidate = candidate_root / "pooled" / factor / f"{factor}-pooled_full_peaks.narrowPeak" if reuse_peaks else call_macs(
            macs3,
            bams,
    ...
    for row in igg_rows.itertuples(index=False):
        candidate = candidate_root / f"rep{row.replicate}" / "IgG" / f"{row.library_id}_full_peaks.narrowPeak" if reuse_peaks else call_macs(
            macs3,
    ...
    source = "existing candidate calls" if reuse_peaks else "BAMs"
    print(f"[DONE] Rebuilt 2020 matched-IgG peak and promoter evidence from {source}.")


def main() -> None:
    parser = argparse.ArgumentParser()
    sources = parser.add_mutually_exclusive_group()
    sources.add_argument("--from-raw", action="store_true")
    sources.add_argument("--from-peaks", action="store_true", help="Recompute binding using existing candidate peak calls and unchanged bigWigs (default)")
    ...
    raw(manifest, reuse_peaks=not args.from_raw)
